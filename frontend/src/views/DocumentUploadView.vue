...
export default {
  ...
  methods: {
    async loadReferenceData() {
      try {
        const [docTypesRes, deptsRes, discsRes, programsRes, sessionsRes] = await Promise.all([
          fetch('/api/document-types'),
          fetch('/api/departments'),
          fetch('/api/disciplines'),
          fetch('/api/edu-programs'),
          fetch('/api/sessions')
        ])
        this.documentTypes = await docTypesRes.json()
        this.departments = await deptsRes.json()
        this.disciplines = await discsRes.json()
        this.eduPrograms = await programsRes.json()
        this.sessions = await sessionsRes.json()
      } catch (error) {
        console.error('Ошибка загрузки справочных данных:', error)
        this.showError('Не удалось загрузить справочные данные')
      }
    },
      ...
      for (const file of this.selectedFiles) {
        try {
          ...
          this.uploadResults.push({
            ...
            message: response.ok ? 'Файл успешно загружен' : result.error || 'Ошибка загрузки',
